...
export async function getSiteDataByPageId({ pageId, from }) {
  ...
  const data = await getOrSetDataWithCache(cacheKey,
    async () => {
      ...
      // 返回并塞入缓存
      return Array.isArray(r?.allPages) ? r : EmptyData(pageId)
    }
...
/**
 * 获取最新文章，按「发布时间」倒序（本站自定义：不用最后修改时间）
 * 修复：原代码用 Object.create(allPosts) 不是真正的数组副本，改为展开运算符
 * 本站改动：改为 publishDate 优先，lastEditedDate 仅作兜底（未设发布时间的文章才用它）
 */
function getLatestPosts({ allPages, from, latestPostCount }) {
  ...
  return [...(allPosts ?? [])]
    .sort((a, b) => {
      const dateA = new Date(a?.publishDate || a?.lastEditedDate)
      const dateB = new Date(b?.publishDate || b?.lastEditedDate)
      return dateB - dateA
    })
